...
Azure では、 9.9.0 リリース以降、複数の Storage VM が Cloud Volumes ONTAP BYOL でサポートされており、アドオンライセンスが付与されています。にアクセスします https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/index.html["Cloud Volumes ONTAP リリースノート"^] 使用している Cloud Volumes ONTAP のバージョンでサポートされる Storage VM 数を確認してください。
