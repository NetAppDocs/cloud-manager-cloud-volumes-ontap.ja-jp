...
1つのノードシステムまたはHAペアに複数のStorage VMを作成できます。HAペアにStorage VMを作成するにはCloud Manager APIを使用する必要があります。一方、CLIまたはSystem Managerを使用して、単一のノードシステムにStorage VMを作成できます。



=== シングルノードシステム

以下の手順では、CLIを使用してシングルノードシステムに新しいStorage VMを作成します。NAS LIF を作成するには 1 つのプライベート IP アドレスが必要で、管理 LIF を作成する場合はもう 1 つのプライベート IP アドレスが必要です。
...
Cloud Manager APIでは、Storage VMの名前変更と削除もサポートされています。
