...
高速の書き込み速度は、すべてのタイプのシングルノードシステムでサポートされています。特定のインスタンスまたは VM タイプを使用する場合は、 AWS と Azure の HA ペアでもサポートされます。GCP の HA ペアでは、高速の書き込み速度はサポートされていません。 https://docs.netapp.com/us-en/cloud-volumes-ontap/["詳細については、 Cloud Volumes ONTAP のリリースノートを参照してください"^]。
