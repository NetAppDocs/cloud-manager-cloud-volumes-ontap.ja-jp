...
.<span class="image"><img src="https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-2.png"[] Alt="2"></span> Cloud Volumes ONTAP とオブジェクトストレージ間の接続を確認します
...
* Cloud Volumes ONTAP では、次のバージョン以降でデータ階層化がサポートされます。
+
...
* GCP では、 SSD 永続ディスク、分散型永続ディスク、または標準的な永続ディスクのいずれかを使用できます。
