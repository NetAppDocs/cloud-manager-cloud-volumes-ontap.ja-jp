...
このページで説明する機能拡張は、 Cloud Volumes ONTAP の管理を可能にする Cloud Manager の機能に固有のものです。リリースノートには、 Cloud Volumes ONTAP ソフトウェア自体の新機能が記載されています。
...
link:task-create-volumes.html["特定のアグリゲートにボリュームを作成する方法について説明します"]。
...
link:task-manage-node-licenses.html#exchange-unassigned-node-based-licenses["未割り当てのノードベースライセンスを交換する方法について説明します"]。
